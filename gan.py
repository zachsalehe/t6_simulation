...
class NoiseGenerator(nn.Module):
    def __init__(self, device, r=320, c=320, n=1):
    ...
    def forward(self, x, t, add_noise):
        ...
        if add_noise:
            ...
            z += torch.randn((*z.shape[:3], 1), device=self.device, requires_grad=True) * self.row

            z += torch.randn((1, *z.shape[1:3], 1), device=self.device, requires_grad=True) * self.rowt

            z += (torch.rand(z.shape, device=self.device, requires_grad=True) - 0.5) * self.quant

            z += torch.randn(z.shape, device=self.device, requires_grad=True) * (self.dark * torch.sqrt(t)) + (self.dark**2 * t)
